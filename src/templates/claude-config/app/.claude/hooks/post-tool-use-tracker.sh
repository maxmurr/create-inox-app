...
detect_repo() {
    ...
    case "$repo" in
        ...
        # Tooling directory
        tooling)
            local tool=$(echo "$relative_path" | cut -d'/' -f2)
            if [[ -n "$tool" ]]; then
                echo "tooling/$tool"
            else
                echo "$repo"
...
get_build_command() {
    ...
    if [[ "$repo" == "database" ]] || [[ "$repo" =~ prisma ]]; then
        if [[ -f "$repo_path/schema.prisma" ]] || [[ -f "$repo_path/prisma/schema.prisma" ]]; then
            echo "cd $repo_path && pnpm exec prisma generate"
            return
        fi
    fi
...
get_tsc_command() {
    ...
    if [[ -f "$repo_path/tsconfig.json" ]]; then
        ...
        if [[ -f "$repo_path/tsconfig.app.json" ]]; then
            echo "cd $repo_path && pnpm exec tsc --project tsconfig.app.json --noEmit"
        else
            echo "cd $repo_path && pnpm exec tsc --noEmit"
        fi
        return
    fi
